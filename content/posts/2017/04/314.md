...
categories:
  - 网络安全
date: "2017-04-17T16:22:51+00:00"
guid: http://www.lucien116.com/?p=314
kratos_love: "1"
parent_post_id: null
post_id: "314"
title: NSA Eternalblue SMB 漏洞分析
url: /archives/314
views: "2174"
...
srv.sys在处理 `SrvOs2FeaListSizeToNt` 的时候逻辑不正确导致越界拷贝。我们首先看下漏洞的触发点：
...
2\. 由于 `SrvOs2FeaListSizeToNt` 中改变了pOs2Fea的length的值，这里使用变大后的值做比较，肯定会越界。
...
在将下面SrvNet分配的对象覆盖前面的a1字节后。由于被覆盖的pool里面存在用于接收数据的buffer的指针。像上面描述的 `0x8d1aa034(0xffd0f020)` 这个地方的指针（我发懒了没有去确定到底是哪一个）。srvnet在接收包的时候就会在固定 `0xffdff000` 这个地址存入客户端发送来的数据。
`0xffdff0000` 这个地址是什么？wrk中有定义，如下：
...
`0xffdff1f1` 处为shellcode.最后在接收完成后，最终调到 `srvnet!SrvNetWskReceiveComplete`.在这个函数中会调用最终的shellcode。可以这么下断点:
...
附件： [dump.bin](https://pastebin.com/RAgsEEiZ)
